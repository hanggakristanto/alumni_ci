...
class Eventa extends CI_Controller
{
    public function __construct()
    {
        parent::__construct();
        $this->load->model(array('Eventa_model' => 'eventa'));
    }
    ...
    public function index()
    {
        if (!$this->ion_auth->logged_in()) {
            ...
            $this->data['_get_eventa'] = $this->eventa->read_per_month();
            if(!isset($this->data['_get_eventa'])||empty($this->data['_get_eventa'])){
                $this->session->set_flashdata('message','Tidak ada Info Kegiatan bulan Ini!');
            }
            $this->data['is_user'] = $this->ion_auth->user()->row();

            $this->data['_view'] = 'eventa/event_list';
    ...
    public function list_admin()
    {
        ...
        $this->data['message'] = (validation_errors() ? validation_errors() : ($this->ion_auth->errors() ? $this->ion_auth->errors() : $this->session->flashdata('message')));
        $this->data['is_user'] = $this->ion_auth->user()->row();
        $this->data['_get_eventa'] = $this->eventa->get_all();
        ...
        ';
        //end partial
        $this->data['_view'] = 'eventa/event_admin_list';
        $this->template->_render_page('layouts/backend', $this->data);
    }
    ...
    public function read($slug)
    {
        if (!$this->ion_auth->logged_in()) {
            ...
            $row = $this->eventa->read_slug($slug);
            if ($row) {
                ...
                $this->data['_view'] = 'eventa/event_read';
                $this->template->_render_page('layouts/backend', $this->data);
            } else {
                $this->session->set_flashdata('message','Data tidak ditemukan');
                redirect(site_url('eventa'));
            }
        }
    }
    ...
    public function create()
    {
        if (!$this->ion_auth->logged_in() || (!$this->ion_auth->is_admin())) {
            ...
            $this->data['action'] = site_url('eventa/tambah_aksi');
            $this->data['id'] = array(
                'name' => 'id',
                'type' => 'hidden',
            ...
            $this->data['_view'] = 'eventa/event_form';
            $this->template->_render_page('layouts/backend', $this->data);
        }
    }
